...
impl<E:Engine> PreparedProver<E> {
    ...
    ) -> Result<(), SynthesisError>
    {
        ...
        let mut witness_poly = Vec::with_capacity(params.domain_h_size);
        witness_poly.extend(prover.input_assignment);
        witness_poly.extend(prover.aux_assignment);

        let domain_h = Domain::<E::Fr>::new_for_size(params.domain_h_size)?;
        let domain_k = Domain::<E::Fr>::new_for_size(params.domain_h_size)?;
        // also no masking for now

        let mut repr = E::Fr::Repr::default();
        ...
        let r_alpha_x_values = eval_unnormalized_bivariate_lagrange_poly_over_different_inputs(alpha, &domain_h, &worker);
        let r_alpha_x_values = Polynomial::from_values(r_alpha_x_values)?;

        let r_alpha_x_poly = r_alpha_x_values.clone().ifft(&worker);

        // now do the same for A/B/C matrixes

        let r_x_x_over_h = eval_unnormalized_bivariate_lagrange_poly_over_diaginal(&domain_h, &worker);
